...
#define DOCTEST_MPI_GEN_ASSERTION(rank_to_test, assertion, ...)                                    \
    static_assert(rank_to_test < test_nb_procs_as_int_constant.value,                              \
                  "Trying to assert on a rank greater than the number of procs of the test!");     \
    if(rank_to_test == test_rank)                                                                  \
    assertion(__VA_ARGS__)

#define DOCTEST_MPI_WARN(rank_to_test, ...)                                                        \
    DOCTEST_MPI_GEN_ASSERTION(rank_to_test, DOCTEST_WARN, __VA_ARGS__)
#define DOCTEST_MPI_CHECK(rank_to_test, ...)                                                       \
    DOCTEST_MPI_GEN_ASSERTION(rank_to_test, DOCTEST_CHECK, __VA_ARGS__)
#define DOCTEST_MPI_REQUIRE(rank_to_test, ...)                                                     \
    DOCTEST_MPI_GEN_ASSERTION(rank_to_test, DOCTEST_REQUIRE, __VA_ARGS__)
...
#define DOCTEST_CREATE_MPI_TEST_CASE(name, nb_procs, func)                                         \
    static void func(DOCTEST_UNUSED int test_rank, DOCTEST_UNUSED int test_nb_procs,               \
    ...
    TEST_CASE(name* doctest::description("MPI_TEST_CASE") *                                        \
              doctest::skip(doctest::insufficient_procs(nb_procs))) {                              \
        doctest::execute_mpi_test_case<nb_procs>(func);                                            \
    }                                                                                              \
    static void func(                                                                              \
            DOCTEST_UNUSED int test_rank, DOCTEST_UNUSED int test_nb_procs,                        \
            DOCTEST_UNUSED MPI_Comm test_comm,                                                     \
