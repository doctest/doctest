...
| ```-spp``` ```--skip-path-prefixes=<bool>``` | Remove the longest matching one of these prefixes from any file path in the output - useful as ```-npf```, but can preserve some context through relative paths. Try: ```--spp=${CMAKE_SOURCE_DIR}/:${CMAKE_BINARY_DIR}/``` |
